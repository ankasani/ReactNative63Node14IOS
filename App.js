...
class App extends Component<{}> {
  constructor() {
    ...
    this.API_URL = JSON.stringify(process.env.API_URL);
  }
  ...
  getUpdateMetadata() {
    CodePush.getUpdateMetadata(CodePush.UpdateState.RUNNING).then(
      (metadata: LocalPackage, remoteMetaData: RemotePackage) => {
        this.setState({
          syncMessage: metadata
            ? JSON.stringify(metadata)
            : `${JSON.stringify(metadata) + JSON.stringify(remoteMetaData)}`,
          progress: false,
        });
      },
      (error: any) => {
    ...
    return (
      <View style={styles.container}>
        <Text style={styles.welcome}>
          {`Welcome to CodePush!!! ${this.API_URL}`}
        </Text>
